...
public interface PreparationApi {
    ...
    /**
     * POST /preparation/collection : Create a Postman collection from an OAS3 specification.
    ...
    @PostMapping(
        value = "/preparation/collection",
        produces = { "application/json" },
        consumes = { "multipart/form-data" }
    )
    default ResponseEntity<Report> createCollection(@ApiParam(value = "") @Valid @RequestPart(value = "filestream", required = true) MultipartFile filestream) {
    ...
    /**
     * POST /preparation/testdata : Create test data for an existing Postman collection.
     * Create test data for an existing Postman collection.
     *
     * @param filestream  (required)
     * @return Test created. (status code 201)
     *         or Bad request. (status code 400)
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 200, message = "unexpected error") })
    @PostMapping(
        value = "/preparation/testdata",
        produces = { "application/json" },
        consumes = { "multipart/form-data" }
    )
    default ResponseEntity<Report> createTestdata(@ApiParam(value = "") @Valid @RequestPart(value = "filestream", required = true) MultipartFile filestream) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"sessionNr\" : 0, \"linkToCollection\" : \"linkToCollection\", \"linkToProperties\" : \"linkToProperties\", \"creationTime\" : \"creationTime\", \"durationSeconds\" : 6, \"linkToReport\" : \"linkToReport\", \"linkToSessions\" : \"linkToSessions\", \"message\" : \"message\", \"error\" : \"error\", \"linkToFeature\" : \"linkToFeature\", \"skipResponseValidation\" : true, \"success\" : true, \"testExecuted\" : true, \"testId\" : \"046b6c7f-0b8a-43b9-b35d-6489e6daee91\", \"startTime\" : \"startTime\", \"endTime\" : \"endTime\" }";
